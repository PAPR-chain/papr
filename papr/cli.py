...
@click.option(
    "--encrypt", default=False, help="Publish as an encrypted file for private review"
)
...
@click.option(
    ...
    "-b",
    default="0.0001",
    help="Amount of LBC to bid in support of the channel claim",
)
...
def create_channel(name, bid, yes):
    ...
    if "error" in data:
        if data["error"]["data"]["name"] == "InsufficientFundsError":
            print(
                "Your account has insufficient funds to create a channel with this bid"
            )
            return "Insufficient Funds"
        else:
            print(
                f"Could not create your account due to an error: {data['error']['data']['name']}"
            )
    else:
